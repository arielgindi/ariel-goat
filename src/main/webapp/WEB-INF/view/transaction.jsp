        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            if (mode === "edit") {
                ...
                formElements.forEach(function (element) {
                    // Disable all except transactionValue (i.e., the “Song Length”)
...
<body>
    <div class="container">
        <h2 class="text-center">
            <c:choose>
                <c:when test="${mode eq 'create'}">Add New Song</c:when>
            ...
            <form action="${mode eq 'create' ? 'saveTransaction' : 'updateTransaction'}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="otherPhoneNumber"
                           value="${transaction.otherPhoneNum}"
                           placeholder="Enter artist name"
                           ${mode eq 'edit' ? 'readonly' : ''} required>
                </div>

                <!-- If edit mode, store ID and show release date -->
                ...
                <!-- “Song Length” -->
                <div class="mb-3">
                    <label class="form-label">Song Length (seconds):</label>
                    <input type="number" class="form-control" name="transactionValue"
                           value="${transaction.transactionValue}"
                           placeholder="Enter length in seconds" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="transactionReason"
                           value="${transaction.transactionReason}"
        ...
        <c:if test="${mode eq 'view'}">
            ...
            <div class="mb-3">
                <label class="form-label"><strong>Song Length (seconds):</strong></label>
            ...
            <!-- If you still want images based on the “song length” value -->
            <c:choose>
                <c:when test="${transaction.transactionValue le 500}">
                    <div class="text-center">
                        <img src="${pageContext.request.contextPath}/resources/images/NoMoney.jpg"
                             class="img-fluid transaction-image">
                    </div>
                </c:when>
                <c:when test="${transaction.transactionValue gt 500 and transaction.transactionValue le 1500}">
                    <div class="text-center">
                        <img src="${pageContext.request.contextPath}/resources/images/MoreMoney.jpg"
                             class="img-fluid transaction-image">
                    </div>
                </c:when>
                <c:when test="${transaction.transactionValue gt 2000}">
                    <div class="text-center">
                        <img src="${pageContext.request.contextPath}/resources/images/Sigma.jpg"
                             class="img-fluid transaction-image">
                    </div>
                </c:when>
                <c:otherwise>
                    <p>No song length set</p>
                </c:otherwise>
            </c:choose>
        </c:if>
